...
static csp* fdpcg_sa(loc_t *xloc, loc_t *saloc, real *saa){
    /**
...
static long *fdpcg_perm(long *nperm, loc_t **xloc, int nps, loc_t *saloc){
    ...
    */
	...
	xdim[ips]=(long)sqrt((real)xloc[ips]->nloc);
...
static csp *fdpcg_prop(long nps, const long *os, long nxg, real dx, 
    ...
    */
	...
	for(long ix=0; ix<nxg; ix++){
		...
		real fx=(jx-nxi2[ips])*dk;
		real fy=(jy-nxi2[ips])*dk;
		...
		case 2:
		    {
			comp shiftx=cexp(cf*(fx*dx));
			comp shifty=cexp(cf*(fy*dx));
...
int main(){
    ...
    real ht[]={0,1000,2000,4000,8000,16000};
    real hs=90000;
    real thetax[]={0,0,-0.0001613797784,-9.973820086e-05,9.973820086e-05,0.0001613797784};
    real thetay[]={0,0.000169684629,5.243545924e-05,-0.0001372777737,-0.0001372777737,5.243545924e-05};
    ...
    writebin(gx,"gx");
    writebin(gy,"gy");
    real dispx[nps],dispy[nps];

    real r0=0.198749305619780;
    comp *invpsd=mycalloc(256*256*6,comp);
    long offset=0;
    for(long ips=0; ips<nps; ips++){
