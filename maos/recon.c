...
void recon_servo_update(sim_t* simu){
	...
	if(((parms->recon.split&&simu->Merr_lo)||parms->evl.split)&&parms->recon.psddtrat_lo>0){//compute PSD on low order control
		...
		if(iframe+1==dtrat){
			...
			{//plot psd ol and cl before updating the gain
				...
				dmat *ep=parms->recon.split?simu->Mint_lo->ep:simu->dmint->ep;
