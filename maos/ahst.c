...
void ngsmod_prep(const parms_t* parms, recon_t* recon, const aper_t* aper, const powfs_t* powfs){
	...
	*/
	...
	switch(parms->tomo.ahst_wt){
	case 0:
		info("NGS mode is not removed from LGS reconstruction output (debug only).\n");
		break;
	case 1:	/*Use gradient weighting. */
		//Done in ngsmod_setup() with Rngs
		break;
	case 2:	/*Retired (was science based weighting to isolate active meta pupil.) */
	case 3: /*Identity weighting. */
		ngsmod->Pngs=dcellpinv(ngsmod->Modes, wts, 1e-14);
		break;
	case 4:/*Remove 2nd order modes from the upper DM and place on ground DM with a scaling factor
		...
		//ngsmod->Pngs=dcellnew(parms->ndm, parms->ndm);
		for(int idm=0; idm<parms->ndm; idm++){
			real ht=parms->dm[idm].ht;
			...
			//P(ngsmod->Pngs, idm, idm)=dpinv2(P(ngsmod->Modes2, idm, idm), wts?P(wts,idm):NULL, 1e-14);
		}
		ngsmod->Pngs=dcellpinv(ngsmod->Modes2, wts, 1e-14);
		break;
	default:
