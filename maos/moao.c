...
void setup_recon_moao(recon_t* recon, const parms_t* parms){
	...
	for(int imoao=0; imoao<nmoao; imoao++){
		...
		P(recon->moao[imoao].aloc,0)=loc_from_map(map, 0);
		...
		P(recon->moao[imoao].HA,0)=mkh(&(propdata_t){.locin=P(recon->moao[imoao].aloc,0), .locout=recon->floc});
...
void moao_recon(sim_t* simu){
	...
	if(1){/*Take High order fitting result */
		if(parms->recon.modal){
			/*dmcommon=dcellnew(parms->ndm, 1);
			for(int idm=0; idm<parms->ndm; idm++){
				dcellmm(&P(dmcommon, idm), P(recon->amod, idm), P(simu->dmrecon,idm), "nn", 1);
			}*/
			dcellmm(&dmcommon, recon->amod, simu->dmrecon, "nn", 1);
		}else{
