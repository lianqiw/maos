...
step6_mvm="$step6_split recon.mvm=1 tomo.alg=1 tomo.precond=1 tomo.maxit=100 fit.alg=0" #MVM
step6_llt="$step6_split powfs0_llt.focus=140"  #LLT implementation
step6_hyst="$step6_split  dm.hyst=[0.05]" #DM hystersis
step6_predict="$step6_split  tomo.predict=1" #prediction (cheat).
...
steps_alg="step6_split step6_mvm step6_llt step6_hyst" 
...
if [ "$test_elt" = 1 ]; then #test elt
	...
	if true; then
		...
		dest+="_morfeo_mk13n"
		atm="atm_mk13n50p.conf atmr.wt=ref:atm.wt atmr.ht=ref:atm.ht"
	fi
else
	base="-c mcao_lgs.conf sim.dt=1/600 sim.dtref=1/600 sim.alhi=0.31 powfs.dsa=[0.4 -2 -1] powfs.siglev=[553 0 0] "
	fovs="34 "
	atm=""
fi
...
if [ "$test" = 1 ] ; then #testing
	...
	base+=" sim.seeds=[1 10 20 30] sim.end=2000"
fi
base+=" save.extra=0"
maos="maos -d"
...
for prof in $profs;do
  if [ "$prof" != "50" ];then
	atm="atm_mk13n${prof}p.conf atmr.wt=ref:atm.wt"
  fi
  for za in $zas;do
	...
	for fov in $fovs ;do
	    for step in $steps_ol $steps_cl $steps_alg ;do
		:
