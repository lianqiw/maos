...
)
{
    ...
    {
	ERROR (CHOLMOD_INVALID, "realloc_multiple: invalid xtype") ;
	return (FALSE) ;
    }
    ...
    if (nint > 0)
    {
	*II = CHOLMOD(realloc) (nnew, sizeof (Int), *II, &i, Common) ;
    }
    if (nint > 1)
    {
	*J = CHOLMOD(realloc) (nnew, sizeof (Int), *J, &j, Common) ;
	...
	if (nold == 0)
	{
	    ...
	    if (nint > 0)
	    {
		*II = CHOLMOD(free) (i, sizeof (Int), *II, Common) ;
	    }
	    if (nint > 1)
	    {
	...
	else
	{
	    if (nint > 0)
	    {
		*II = CHOLMOD(realloc) (nold, sizeof (Int), *II, &i, Common) ;
	    }
	    if (nint > 1)
	    {
