 ...
 *	G3) and (W0 through W3).  j=27, jj1=28, j2=30, and j3=31.
 *
 *	G0	      x  x  x  x  x  .  .  .
 *	G1	      x  x  x  x  x  .  .  .
 *	G3	      x  x  x  x  x  .  .  .
...
#define w(row,col) W [WDIM*(row) + (col)]
    ...
    Int *Li, *Lp, *Lnz ;
    Int jj1, j2, j3, p0, p1, p2, p3, parent, lnz, pend, k ;
    Int use_dbound = IS_GT_ZERO (Common->dbound) ;
	...
	if (parent <= e && lnz == Lnz [parent] + 1)
	{
	    ...
	    /* -------------------------------------------------------------- */
	    /* node j and its parent jj1 can be updated at the same time */
	    /* -------------------------------------------------------------- */

	    jj1 = parent ;
	    j2 = (lnz > 2) ? (Li [p0+1]) : Int_max ;
	    j3 = (lnz > 3) ? (Li [p0+2]) : Int_max ;
	    W1 = W + WDIM * jj1 ;	/* pointer to row jj1 of W */
	    p1 = Lp [jj1] ;
		...
		/* p0 and p1 currently point to row j2 in cols j and jj1 of L */
		...
		/* update L (j2,j) and update L (j2,jj1) */
		{
		    double lx [2] ;
		    lx [0] = Lx [p0] ;
		    lx [1] = Lx [p1] ;
		...
		/* update L (j3,j), L (j3,jj1), and L (j3,j2) */
		{
		    double lx [3] ;
		    lx [0] = Lx [p0] ;
		    lx [1] = Lx [p1] ;
		...
		/* each iteration updates L (i, [j jj1 j2 j3]) */
