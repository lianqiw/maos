...
loc_t* locnew(long nloc, real dx, real dy){
	...
	if(nloc>0){
		...
		loc->nref=mycalloc(1, _Atomic(int)); 
		atomic_init(loc->nref, 1);
	}
	loc->nloc=nloc;
	loc->two=2;
...
pts_t* ptsnew(long nsa, real dsax, real dsay, long nxsa, long nysa, real dx, real dy){
	...
	pts->dsax=dsax;
	pts->dsay=dsay;
	pts->nref=mycalloc(1, _Atomic(int)); 
	atomic_init(pts->nref, 1);
	pts->nxsa=nxsa;
	pts->nysa=nysa;
	pts->dx=dx;
	pts->dy=dy;
