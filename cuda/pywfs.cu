...
dsp *gpu_pywfs_mkg(const PARMS_T *parms, const POWFS_T *powfs, loc_t *aloc, int iwfs, int idm){
    ...
    pywfs_grad(grad0, ints, cupowfs[ipowfs].saa, cuwfs[iwfs].isum, cupowfs[ipowfs].pyoff, powfs[ipowfs].pywfs->gain, stream);
	...
	pywfs_grad(grad, ints, cupowfs[ipowfs].saa, cuwfs[iwfs].isum, cupowfs[ipowfs].pyoff, powfs[ipowfs].pywfs->gain,stream);
