...
void mvm_test(int igpu){
...

	{
		...
		for(int i=0; i<N; i+=iN){
			...
			DO(CUBL(gemv)(stream[0].blas(), CUBLAS_OP_N, M, nleft, &one, cumvm()+i*M, M, cux()+i, 1, &one, cuy(), 1));
		}
		event[1].record(stream[0]);
		stream[0].sync();
		DO(cudaEventElapsedTime(&tm, event[0], event[1]));
		dbg("cublas?gemv takes %.6f ms\n", tm);
		cuwrite(cuy, stream[0], "y_cugemv");
	}
	{
		...
		cuwrite(cuy, stream[0], "y_multimv");
	}
	{
	//custom method
		int naeach, mp_count;
	...
	for(int is=0; is<nstream; is++){
		...
		(cumvm()+M*N/nstream*is, M, N/nstream);
