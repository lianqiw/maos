...
dmat *psdt2s(const dmat *psdt, real vmean){
    ...
    real alpha=psdt->p[0];//power
    real beta=psdt->p[1];//strength
    real alpha2=alpha-1;
    //n is -alpha2
...
dmat *psds2t(const dmat *psds, real vmean){
    ...
    real alpha=psds->p[0];//power
    real beta=psds->p[1];//strength
    real alpha2=alpha+1;
    //n is -alpha
