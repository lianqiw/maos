...
*/
/*
  This file is obtained from and modified for Linux
...
/*The following is copied from pthread.h to put in inner
  cleanup_push to avoid warning of shadowing variables*/
# define pthread_cleanup_push2(routine, arg)				\
    do {								\
    __pthread_unwind_buf_t __cancle_buf2;				\
    void (*__cancel_routine2) (void *) = (routine);			\
    void *__cancel_arg2 = (arg);					\
    int not_first_call2 = __sigsetjmp ((struct __jmp_buf_tag *) (void *)	\
				      __cancle_buf2.__cancel_jmp_buf, 0); \
    if (__builtin_expect (not_first_call2, 0))				\
	{								\
	    __cancel_routine2 (__cancel_arg2);				\
	    __pthread_unwind_next (&__cancle_buf2);			\
	    /* NOTREACHED */						\
	}								\
									\
    __pthread_register_cancel (&__cancle_buf2);				\
    do {
...
# define pthread_cleanup_pop2(execute)					\
    do { } while (0);/* Empty to allow label before pthread_cleanup_pop.  */ \
    } while (0);							\
    __pthread_unregister_cancel (&__cancle_buf2);			\
    if (execute)							\
	__cancel_routine2 (__cancel_arg2);				\
    } while (0)
/*end copied*/
...
worker_thread(void *arg)
{
	...
	pthread_cleanup_push((void (*)(void *))worker_cleanup, (void*)pool);
	...
	for (;;) {
		...
		if ((job = pool->pool_head) != NULL) {
			...
			(void) pthread_mutex_unlock(&pool->pool_mutex);
			pthread_cleanup_push2((void (*)(void *))job_cleanup,(void*) pool);
			free(job);
			/*
			 ...
			 */
			pthread_cleanup_pop2(1);	/* job_cleanup(pool) */
			//job_cleanup(pool);
		}
